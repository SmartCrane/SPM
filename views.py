...
from SPM.settings import GERDECODEURL
...
def outsp(request):
    ...
    userdeptid = ViewBdataCommonUserOrg.objects.filter(usercode=request.user.username).order_by(
        'orglevel').first().orgid
    try:
...
def insert_database(request):
    ...
    try:
        ...
        currentuser = TabBdataCommonUser.objects.get(usercode=request.user.username)
        userdeptid = ViewBdataCommonUserOrg.objects.filter(usercode=currentuser.usercode).order_by(
            'orglevel').first().orgid
        if facinspnum:
        ...
        for one in jsondata:
            if one.get('spname') and one.get('sid') and one.get('amount'):
                ...
                if isimport:  # 如果是导入模板的方式，则保存一份在临时表
                    ...
                    odsinspinfo.save()
        response = HttpResponse(json.dumps({"status": "ok", "recordid": recordid}, ensure_ascii=False),
...
def get_org_data(request):
    ...
    if parentorg:
        ...
        for oneorg in allorg:
            ...
            orginfo = {"orgname": oneorg.orgname, "org_order": oneorg.org_order, "org_dic": orgdic.orgcode,
